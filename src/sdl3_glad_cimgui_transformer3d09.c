// nope fail to move with parent child as independ error in code update transform.
...
typedef struct {
    ...
    versor rotation; // cglm quaternion (versor)
    ...
    ecs_entity_t parent; // Explicit parent entity reference (optional, for clarity)
} Transform3D;
...
void update_transform_system(ecs_iter_t *it) {
    ...
    ecs_query_t *query_child = ecs_query(it->world, {
        .terms = {{ .id = ecs_pair(EcsChildOf, EcsWildcard) }}
    });
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        mat4 rot_mat;
        glm_quat_mat4(transform->rotation, rot_mat);
        ...
        while (ecs_query_next(&child_it)) {
            if (ecs_field_id(&child_it, 0) == it->entities[i]) { // Compare parent entity ID
...
// Extract Euler angles (ZYX order) from a rotation matrix, returns radians
void custom_euler_zyx(mat4 rot_mat, vec3 euler) {
    // Assuming rot_mat is a rotation matrix (no scaling or translation)
    euler[1] = asinf(-rot_mat[2][0]); // Pitch (Y)
    float cos_pitch = cosf(euler[1]);

    if (fabsf(cos_pitch) > 0.0001f) { // Avoid division by zero
        euler[0] = atan2f(rot_mat[2][1] / cos_pitch, rot_mat[2][2] / cos_pitch); // Roll (Z)
        euler[2] = atan2f(rot_mat[1][0] / cos_pitch, rot_mat[0][0] / cos_pitch); // Yaw (X)
    } else {
        euler[0] = 0.0f; // Roll is arbitrary when pitch is ±90°
        euler[2] = atan2f(-rot_mat[0][1], rot_mat[1][1]); // Yaw
    }
}
...
int main() {
    ...
    versor parent_rot;
    glm_quat_identity(parent_rot); // Zero rotation
    ...
    ecs_set(world, parent, Transform3D, {
        ...
        .rotation = {parent_rot[0], parent_rot[1], parent_rot[2], parent_rot[3]},
    ...
    versor child_rot;
    vec3 child_euler = {glm_rad(0.0f), glm_rad(45.0f), glm_rad(0.0f)}; // 45 degrees around Y
    glm_euler_zyx_quat(child_euler, child_rot);
    ...
    ecs_set(world, child, Transform3D, {
        ...
        .rotation = {child_rot[0], child_rot[1], child_rot[2], child_rot[3]},
        .scale = {1.0f, 1.0f, 1.0f},
        .isDirty = true,
        .parent = parent
    });
    ...
    // Create a grandchild cube for testing deeper hierarchy
    versor grandchild_rot;
    vec3 grandchild_euler = {glm_rad(0.0f), glm_rad(0.0f), glm_rad(45.0f)}; // 45 degrees around Z
    glm_euler_zyx_quat(grandchild_euler, grandchild_rot);

    ecs_entity_t grandchild = ecs_entity(world, { .name = "GrandchildCube" });
    ecs_set(world, grandchild, Transform3D, {
        .position = {0.5f, 0.5f, 0.0f},
        // .rotation = {0.0f, 0.0f, 45.0f},
        .rotation = {grandchild_rot[0], grandchild_rot[1], grandchild_rot[2], grandchild_rot[3]},
    ...
    Transform3DContext* contexts = NULL; // Persist array between frames
        ...
        // Transform 3D Context and list
        {
            ...
            if (selected_id != 0) {
                ...
                if (transform) {
                    ...
                    mat4 rot_mat;
                    glm_quat_mat4(transform->rotation, rot_mat);
                    custom_euler_zyx(rot_mat, euler); // Returns radians
                    euler[0] = glm_deg(euler[0]); // Convert to degrees
                    euler[1] = glm_deg(euler[1]);
                    euler[2] = glm_deg(euler[2]);
                    ...
                    // Rotation (Euler angles in UI, converted to quaternion)
                    igText("Rotation (Euler Angles)");
                    float euler_temp[3] = {euler[0], euler[1], euler[2]};
                    changed |= igSliderFloat("X##rot", &euler_temp[0], -360.0f, 360.0f, "%.1f", 0);
                    changed |= igSliderFloat("Y##rot", &euler_temp[1], -360.0f, 360.0f, "%.1f", 0);
                    changed |= igSliderFloat("Z##rot", &euler_temp[2], -360.0f, 360.0f, "%.1f", 0);
                    ...
                    // If rotation changed, convert Euler angles back to quaternion
                    if (changed && (euler_temp[0] != euler[0] || euler_temp[1] != euler[1] || euler_temp[2] != euler[2])) {
                        vec3 euler_rad = {glm_rad(euler_temp[0]), glm_rad(euler_temp[1]), glm_rad(euler_temp[2])};
                        glm_euler_zyx_quat(euler_rad, transform->rotation);
                        glm_quat_normalize(transform->rotation); // Ensure quaternion is normalized
                    }
