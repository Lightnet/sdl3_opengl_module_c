...
void update_transform_system(ecs_iter_t *it) {
    ...
    for (int i = 0; i < it->count; i++) {
        ...
        glm_rotate_x(local, glm_rad(transform->rotation[0]), local);
        glm_rotate_y(local, glm_rad(transform->rotation[1]), local);
        glm_rotate_z(local, glm_rad(transform->rotation[2]), local);
        ...
        while (ecs_query_next(&child_it)) {
            if (ecs_term_str(&child_it, 1) == it->entities[i]) { // Match children of current entity
    ...
    ecs_query_fini(query_child); // Clean up query
}
